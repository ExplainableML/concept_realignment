...
def produce_addition_set(
    ...
    noise_level=0.0,
    consecutive_concepts_same_probability=0.0,
):
    print("GENERATING MNIST DATASET WITH CONSECUTIVE CONCEPTS BEING EQUAL WITH PROBABILITY ", consecutive_concepts_same_probability)
    ...
    for i in range(dataset_size):
        ...
        sample_label = 0
        selected = []
        # print("NUM SELECTED DIGITS = ", len(selected_digits))
        for i, operand_digits, remap, total_samples, total_labels in zip(
            range(len(selected_digits)),
            selected_digits,
            operand_remaps,
            total_operand_samples,
            total_operand_labels,
        ):
            # if concept is odd, make it the same as the previous concept with probability p
            if i % 2 == 1 and random.random() < consecutive_concepts_same_probability:
                print("Using same concept as before")
                img_idx = prev_idx
            else:
...
def load_mnist_addition(
    ...
    x_test, y_test, c_test = produce_addition_set(
        ...
        consecutive_concepts_same_probability=consecutive_concepts_same_probability,
    )
...
def generate_data(
    ...
    train_dl, val_dl, test_dl = load_mnist_addition(
        ...
        consecutive_concepts_same_probability=config["consecutive_concepts_same_probability"],
    )
