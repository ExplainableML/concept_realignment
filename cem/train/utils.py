...
def wrap_pretrained_model(c_extractor_arch, pretrain_model=True, extra_layers=False):
    def _result_x2c_fun(output_dim):
        if extra_layers:
            model = c_extractor_arch(pretrained=pretrain_model, output_dim=output_dim)
            return model
