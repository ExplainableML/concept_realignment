...
#         print("include certainty = ", self.include_certainty)
#         print("CONCEPT LOSS = ", concept_loss)
#         print("TASK LOSS = ", intervention_task_loss)
...
class IntAwareConceptBottleneckModel(ConceptBottleneckModel):
    def __init__(
        ...
        print("SIGMOIDAL PROBS = ", sigmoidal_prob)
        ...
        self.concept_corrector_layers = nn.ModuleList([nn.Linear(self.num_concepts, self.num_concepts),
                                                       nn.Linear(self.num_concepts, self.num_concepts),
                                                    #    nn.Linear(self.num_concepts, self.num_concepts),
                                                       ])

        self.concept_corrector_batchnorms = nn.ModuleList([nn.BatchNorm1d(self.num_concepts) for _ in range(2)])
        self.concept_corrector_output_layer = nn.Linear(self.num_concepts, self.num_concepts)
        self.apply_concept_corrector = apply_concept_corrector
        print("APPLY CONCEPT CORRECTOR = ", self.apply_concept_corrector)


    def concept_corrector(self, concepts, already_intervened_concepts):
        inputs = concepts
        for layer, batchnorm in zip(self.concept_corrector_layers, self.concept_corrector_batchnorms):
            concepts = layer(concepts)
            concepts = batchnorm(concepts)
            concepts = torch.relu(concepts)

        concepts = self.concept_corrector_output_layer(concepts)
        concepts = torch.sigmoid(concepts)
        concepts = already_intervened_concepts * inputs + (1 - already_intervened_concepts) * concepts
...
#                 competencies,
    ...
    def _run_step(
        ...
        print('CALLED _FORWARD!')
        c_sem, c_logits, y_logits = outputs[0], outputs[1], outputs[2]

        # print('use sigmoid = ', self.sigmoidal_prob)
        print('intervention_idx = ', intervention_idxs)
        print('RANGE OF PREDICTED CONCEPTS = ')
        print(torch.min(c_sem), torch.max(c_sem))

        if c_sem.isnan().any() == True:
            ...
            print('intervention_idx = ', intervention_idxs)
            print('c = ', c)
            print('any nan in c? = ', c.isnan().any())
            print('y = ', y)
            print('any nan in y? = ', y.isnan().any())

            print('OUTPUTS = ')
            print('c_sem = ', c_sem)
            print('c_logits = ', c_logits)
        ...
        # if self.apply_concept_corrector:
        #     c_sem_corrected = self.concept_corrector(c_sem, torch.zeros_like(c_sem))

        #     embeddings = (
        #         (
        #             pos_embeddings *
        #             torch.unsqueeze(c_sem_corrected, dim=-1)
        #         ) + (
        #             neg_embeddings *
        #             (1 - torch.unsqueeze(c_sem_corrected, dim=-1))
        #         )
        #     )
        ...
        #     print('c_sem.size() = ', c_sem.size())
        #     print('pos_embeddings.size() = ', pos_embeddings.size())

        #     y_logits = self.c2y_model(embeddings)
        ...
        print('train = ', train)
        print('intervention_idxs = ', intervention_idxs)
        ...
        if (
            ...
            if prev_num_of_interventions != int_basis_lim:
                ...
                print('HELLO')
                print(int(horizon_lim), self.max_horizon, int_basis_lim - prev_num_of_interventions - initially_selected)
            ...
            print("FIRST TASK DISCOUNT = ", first_task_discount)
            if self.include_task_trajectory_loss and (
                self.task_loss_weight == 0
            ):
                # Then we initialize the intervention trajectory task loss to
                ...
                if self.apply_concept_corrector:
                    concept_corrector_output_init = self.concept_corrector(c_sem, torch.zeros_like(c_sem))                
                    intervention_concept_loss = self.loss_concept(concept_corrector_output_init, c)
                    concept_corrector_loss_init = intervention_concept_loss.item()
            ...
            # all_probs = []  # IN THIS LIST, WE WILL STORE ALL CONCEPT VALUES THAT WERE FED TO THE CLASSIFIER, ONE ENTRY FOR EACH TIMESTEP
            print('NUM ROLLOUTS = ', num_rollouts)
            ...
            for _ in range(num_rollouts):
                for i in range(current_horizon):
                    ...
                    if self.include_task_trajectory_loss and (
                        ...
                        print("INTERVENTION IDXS = ", intervention_idxs.size(), torch.sum(intervention_idxs, dim=1))

                        # APPLY CONCEPT CORRECTION
                        if self.apply_concept_corrector:
                            print("APPLYING CONCEPT CORRECTION")
                            c_loss_without_correction = self.loss_concept(probs, c).item()

                            probs = self.concept_corrector(probs, intervention_idxs)
                            c_loss = self.loss_concept(probs, c)

                            concept_corrector_loss_final = c_loss.item()

                            if self.average_trajectory:
                                print('ADDING TO CONCEPT LOSS')
                                intervention_concept_loss += task_discount * c_loss / task_trajectory_weight
                            else:
                                intervention_concept_loss = task_discount * c_loss

                            # if self.average_trajectory:
                            #     intervention_concept_loss_numerator += 10*(task_discount * c_loss)/task_trajectory_weight
                            #     intervention_concept_loss_denominator = 10*task_discount/task_trajectory_weight
                            #     print(f'task_discount = {task_discount}, task_trajectory_weight = {task_trajectory_weight}')
                            # else:
                            #     intervention_concept_loss_numerator += task_discount * c_loss
                            #     intervention_concept_loss_denominator = task_discount

                            # print("NUMERATOR REQUIRES GRAD = ", intervention_concept_loss_numerator.requires_grad)
            ...
            intervention_loss_scalar = 0.0
            intervention_concept_loss = 0.0
            concept_corrector_loss_init = None
            concept_corrector_loss_final = None
            c_loss_without_correction = None
        ...
        if self.concept_loss_weight != 0:
            ...
            # whenever no concept supervision is provided
            if self.include_certainty:
                concept_loss_original = self.loss_concept(c_sem, c)

                if self.apply_concept_corrector:
                    concept_loss = concept_loss_original
                    # concept_loss = (concept_loss_original + intervention_concept_loss_numerator)/intervention_concept_loss_denominator
                    # concept_loss = intervention_concept_loss_numerator/intervention_concept_loss_denominator
                else:
                    concept_loss = concept_loss_original
        ...
        loss = (
            self.concept_loss_weight * (concept_loss + intervention_concept_loss)/2 +
            # self.concept_loss_weight * (intervention_concept_loss) +
            self.intervention_weight * intervention_loss +
            self.task_loss_weight * task_loss +
            self.intervention_task_loss_weight * intervention_task_loss
        )

        print("CONCEPT LOSS ORIGINAL = ", concept_loss_original)
        print("CONCEPT LOSS = ", concept_loss)
        print("CONCEPT INTERVENTION LOSS = ", intervention_concept_loss)
        if self.apply_concept_corrector:
            print(f"concept_loss_init = {concept_loss}, concept_loss_after_intervention (no correction) = {c_loss_without_correction}")
            print(f"concept_corrector_loss_init = {concept_corrector_loss_init}, concept_corrector_loss_final = {concept_corrector_loss_final}")
...
class IntAwareConceptEmbeddingModel(
    ...
    def __init__(
        ...
        self.apply_concept_corrector = True

        self.concept_corrector_layers = nn.ModuleList([nn.Linear(self.n_concepts, self.n_concepts),
                                                       nn.Linear(self.n_concepts, self.n_concepts),
                                                    #    nn.Linear(self.num_concepts, self.num_concepts),
                                                       ])

        self.concept_corrector_batchnorms = nn.ModuleList([nn.BatchNorm1d(self.n_concepts) for _ in range(2)])
        self.concept_corrector_output_layer = nn.Linear(self.n_concepts, self.n_concepts)
        self.apply_concept_corrector = apply_concept_corrector
        print("APPLY CONCEPT CORRECTOR = ", self.apply_concept_corrector)
