...
from cem.models.c_extractor import BackboneWithExtraLayers
...
def construct_model(
    ...
    if config["architecture"] in ["ConceptEmbeddingModel", "MixtureEmbModel"]:
        ...
        extra_params = {
            "apply_concept_corrector": config.get("apply_concept_corrector", False),
            "bool": config["bool"],
    ...
    if isinstance(config["c_extractor_arch"], str):
        ...
        elif config["c_extractor_arch"] == "backbone_with_extra_layers":
            c_extractor_arch = lambda pretrained, output_dim: BackboneWithExtraLayers(backbone_name=config["c_extractor_backbone"], num_hidden_layers=config["c_extractor_num_extra_layers"], pretrained=pretrained, output_dim=output_dim)
    ...
    return model_cls(
        ...
        c_extractor_arch=utils.wrap_pretrained_model(c_extractor_arch, extra_layers=(config["c_extractor_arch"] == "backbone_with_extra_layers")),
...
def construct_sequential_models(
    ...
    if isinstance(config["c_extractor_arch"], str):
        if config["c_extractor_arch"] == "resnet18":
            ...
            c_extractor_arch = BackboneWithExtraLayers
        else:
            raise ValueError(
    ...
    if config['c_extractor_arch'] == 'backbone_with_extra_layers':
        x2c_model = BackboneWithExtraLayers(backbone_name=config['c_extractor_backbone'], output_dim=n_concepts, num_hidden_layers=config['c_extractor_num_extra_layers'], pretrained=config.get('pretrain_model', True))
    else:
        try:
